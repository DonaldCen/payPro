...
public class MerchantServiceImpl extends BaseService<Merchant> implements MerchantService {
    ...
    public FebsResponse merchantApply(MerchantRegisterVo vo){
        ...
        if(params[1].containsKey("bankId")) params[1].remove("bankId");
        if(params[1].containsKey("rateId")) params[1].remove("rateId");
        ...
        try {
           ...
           // log.info("签名验证标识"+flag);
            checkParam( params[1]);
            // 先保存申请数据到数据库中(状态 待签中)
            params[0].put("status", SignStatusEnum.AUDITING.getValue());
            //获取新的主键ID
